...
const rows = merged.map((m) => {
  ...
  const context = `${Math.round((m.contextWindow || 0) / 1000)}K`;
  const output = `${Math.round((m.maxTokens || 0) / 1000)}K`;
  const notes = m.notes || "";
  return `| ${m.name} | \`${m.id}\` | ${reasoning} | ${input} | ${context} | ${output} | ${notes} |`;
});
...
if (startIdx === -1 || endIdx === -1) {
  console.error(
    "README.md is missing <!-- MODELS_TABLE_START --> or <!-- MODELS_TABLE_END --> markers."
  );
  process.exit(1);
}
